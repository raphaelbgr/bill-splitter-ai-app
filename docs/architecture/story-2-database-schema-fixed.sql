-- ============================================================================
-- RachaAI - Story 2: Database Foundation Schema (Supabase Compatible)
...
-- Non-partitioned table for simplicity (Supabase handles scaling)
...
-- Non-partitioned table for simplicity
...
-- Groups: users can access groups they're members of
...
-- Group members: simplified policies to prevent recursion
...
-- Conversations: users can access conversations in their groups
...
-- Expenses: users can access expenses in their groups
...
-- Expense participants: users can view participants of expenses they have access to
...
CREATE POLICY "Users can view own data access log" ON data_access_log
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can view own processing records" ON processing_records
    FOR SELECT USING (auth.uid() = user_id);
