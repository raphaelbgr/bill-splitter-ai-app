...
// List of internationalization endpoints to update
const internationalizationEndpoints = [
...
// Template for updated internationalization endpoints
const internationalizationEndpointTemplate = (endpointName) => {
...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    return res.status(200).json({
      success: true,
      data: {
        ${camelCaseName}: 'mock-internationalization-data'
      }
    });
  }
...
// Update each internationalization endpoint
internationalizationEndpoints.forEach(endpointPath => {
  ...
  const content = internationalizationEndpointTemplate(endpointName);
  fs.writeFileSync(fullPath, content);
  console.log(`Updated: ${endpointPath}`);
});

console.log('All internationalization endpoints updated successfully!'); 